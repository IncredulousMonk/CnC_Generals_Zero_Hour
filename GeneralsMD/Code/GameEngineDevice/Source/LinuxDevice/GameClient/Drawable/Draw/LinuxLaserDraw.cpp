...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
LinuxLaserDrawModuleData::LinuxLaserDrawModuleData()
{
   m_ini.m_innerBeamWidth = 0.0f;         //The total width of beam
   m_ini.m_outerBeamWidth = 1.0f;         //The total width of beam
   m_ini.m_numBeams = 1;                  //Number of overlapping cylinders that make the beam. 1 beam will just use inner data.
   m_ini.m_maxIntensityFrames = 0;        //Laser stays at max intensity for specified time in ms.
   m_ini.m_fadeFrames = 0;                //Laser will fade and delete.
   m_ini.m_scrollRate = 0.0f;
   m_ini.m_tile = false;
   m_ini.m_segments = 1;
   m_ini.m_arcHeight = 0.0f;
   m_ini.m_segmentOverlapRatio = 0.0f;
   m_ini.m_tilingScalar = 1.0f;
}
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void LinuxLaserDrawModuleData::buildFieldParse(void* what, MultiIniFieldParse& p) 
{
   ModuleData::buildFieldParse(what, p);

   static const FieldParse dataFieldParse[] = 
   {
      { "NumBeams",              INI::parseUnsignedInt,           NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_numBeams) },
      { "InnerBeamWidth",        INI::parseReal,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_innerBeamWidth) },
      { "OuterBeamWidth",        INI::parseReal,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_outerBeamWidth) },
      { "OuterColor",            INI::parseColorInt,              NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_outerColor) },
      { "InnerColor",            INI::parseColorInt,              NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_innerColor) },
      { "MaxIntensityLifetime",  INI::parseDurationUnsignedInt,   NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_maxIntensityFrames) },
      { "FadeLifetime",          INI::parseDurationUnsignedInt,   NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_fadeFrames) },
      { "Texture",               INI::parseAsciiString,           NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_textureName) },
      { "ScrollRate",            INI::parseReal,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_scrollRate) },
      { "Tile",                  INI::parseBool,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_tile) },
      { "Segments",              INI::parseUnsignedInt,           NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_segments) },
      { "ArcHeight",             INI::parseReal,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_arcHeight) },
      { "SegmentOverlapRatio",   INI::parseReal,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_segmentOverlapRatio) },
      { "TilingScalar",          INI::parseReal,                  NULL, offsetof(LinuxLaserDrawModuleData::IniData, m_tilingScalar) },
      { 0, 0, 0, 0 }
   };
   LinuxLaserDrawModuleData* self {static_cast<LinuxLaserDrawModuleData*>(what)};
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
LinuxLaserDraw::LinuxLaserDraw(Thing *thing, const ModuleData* moduleData): DrawModule(thing, moduleData),
   m_line3D(NULL),
   m_texture(NULL),
   m_textureAspectRatio(1.0f),
   m_selfDirty(TRUE)
{
DEBUG_LOG(("LinuxLaserDraw::LinuxLaserDraw not yet implemented!\n"));
#if 0
   Vector3 dummyPos1( 0.0f, 0.0f, 0.0f );
   Vector3 dummyPos2( 1.0f, 1.0f, 1.0f );
   Int i;

   const LinuxLaserDrawModuleData *data = getLinuxLaserDrawModuleData();

   m_texture = WLinuxAssetManager::Get_Instance()->Get_Texture( data->m_textureName.str() );
   if (m_texture)
   {
   ...
   for( int segment = 0; segment < data->m_segments; segment++ )
   {
      ...
      for( i = data->m_numBeams - 1; i >= 0; i-- )
      {
         ...
         if( line )
         {
            ...
            // add to scene
            LinuxDisplay::m_3DScene->Add_Render_Object( line );	//add it to our scene so it gets rendered with other objects.
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
LinuxLaserDraw::~LinuxLaserDraw( void )
{
DEBUG_LOG(("LinuxLaserDraw::~LinuxLaserDraw not yet implemented!\n"));
#if 0
   const LinuxLaserDrawModuleData *data = getLinuxLaserDrawModuleData();
   ...
   for( int i = 0; i < data->m_numBeams * data->m_segments; i++ )
   {
      ...
      LinuxDisplay::m_3DScene->Remove_Render_Object( m_line3D[ i ] );
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
Real LinuxLaserDraw::getLaserTemplateWidth() const
{
   const LinuxLaserDrawModuleData *data = getLinuxLaserDrawModuleData();
   return data->m_ini.m_outerBeamWidth * 0.5f;
}

//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void LinuxLaserDraw::doDrawModule(const Matrix3D* transformMtx)
{
(void) transformMtx;
DEBUG_CRASH(("LinuxLaserDraw::doDrawModule not yet implemented!"));
#if 0
   ...
   if( !update )
   {
      DEBUG_ASSERTCRASH( 0, ("LinuxLaserDraw::doDrawModule() expects its owner drawable %s to have a ClientUpdate = LaserUpdate module.", draw->getTemplate()->getName().str() ));
      return;
   }
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void LinuxLaserDraw::crc(Xfer *xfer)
{
...
/** Xfer method
   ...
   * 1: Initial version */
// ------------------------------------------------------------------------------------------------
void LinuxLaserDraw::xfer(Xfer *xfer)
{
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void LinuxLaserDraw::loadPostProcess( void )
{
