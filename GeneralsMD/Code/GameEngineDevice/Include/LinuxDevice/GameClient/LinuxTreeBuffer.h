...
#ifndef __LINUXTREE_BUFFER_H_
#define __LINUXTREE_BUFFER_H_
...
//-----------------------------------------------------------------------------
class MeshClass;
class LinuxTreeBuffer; 
class TileData;
class LinuxTreeDrawModuleData;
struct BreezeInfo;
class GeometryInfo;
class LinuxProjectedShadow;

//-----------------------------------------------------------------------------
//           Type Defines
//-----------------------------------------------------------------------------

enum LinuxToppleState
{
   TOPPLE_UPRIGHT = 0,
   TOPPLE_FALLING,
   TOPPLE_FOGGED,
...
typedef struct {
   ...
   LinuxToppleState m_toppleState;						///< Stage this module is in.
...
typedef struct {
   ...
   const LinuxTreeDrawModuleData *m_data;
...
//
// LinuxTreeBuffer: Draw buffer for the trees.
//
//
class LinuxTreeBuffer : public Snapshot
{
   //friend class BaseHeightMapRenderObjClass;

   //-----------------------------------------------------------------------------
   //                             LinuxTreeTextureClass
   //-----------------------------------------------------------------------------
   class LinuxTreeTextureClass : public TextureClass
   {
      W3DMPO_GLUE(LinuxTreeTextureClass)
   protected:
      virtual void Apply(unsigned int stage);

   public:
         /// Create texture.
         LinuxTreeTextureClass(unsigned width, unsigned height);

         // just use default destructor. ~TerrainTextureClass(void);
   public:
      int update(LinuxTreeBuffer *buffer); ///< Sets the pixels, and returns the actual height of the texture.
      void setLOD(Int LOD) const;
   };

public:

   LinuxTreeBuffer(void);
   ~LinuxTreeBuffer(void); 
   /// Add a tree at location.  Name is the Linux model name.
   ...
   /// Add a type of tree.  Name is the Linux model name.
   Int addTreeType(const LinuxTreeDrawModuleData *data);
...
private:
   ...
   Bool		m_isTerrainPass;  ///< True if the terrain was drawn in this Linux scene render pass.
...
#endif  // end __LINUXTREE_BUFFER_H_
