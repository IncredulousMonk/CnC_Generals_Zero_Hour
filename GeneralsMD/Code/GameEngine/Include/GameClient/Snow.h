...
class WeatherSetting : public Overridable
{
	...
	public:
		// MG: Cannot apply offsetof to WeatherSetting, so had to move data into an embedded struct.
		struct Data
		{
	...
	public:
		WeatherSetting()
		{
			m_data.m_snowTexture = "EXSnowFlake.tga";
			m_data.m_snowFrequencyScaleX=0.0533f;
			m_data.m_snowFrequencyScaleY=0.0275f;
			m_data.m_snowAmplitude=5.0f;
			m_data.m_snowPointSize=1.0f;
			m_data.m_snowQuadSize=0.5f;
			m_data.m_snowBoxDimensions=200;
			m_data.m_snowBoxDensity=1;
			m_data.m_snowVelocity=4;
			m_data.m_usePointSprites=TRUE;
			m_data.m_snowEnabled=FALSE;
			m_data.m_snowMaxPointSize=64.0f;
			m_data.m_snowMinPointSize=0.0f;
		}
...
class SnowManager : public SubsystemInterface
{
  public :
	...
	// No copies allowed!
	SnowManager(const SnowManager&) = delete;
	SnowManager& operator=(const SnowManager&) = delete;
