...
//-------------------------------------------------------------------------------------------------
LinuxAnimationInfo::LinuxAnimationInfo(const AsciiString& name, Bool isIdle, Real distanceCovered) : 
   m_name(name),
...
//-------------------------------------------------------------------------------------------------
LinuxAnimationInfo::LinuxAnimationInfo( const LinuxAnimationInfo &r ) : 
   m_name(r.m_name),
#ifdef RETAIN_ANIM_HANDLES
   m_handle(r.m_handle),
#endif
...
//-------------------------------------------------------------------------------------------------
LinuxAnimationInfo& LinuxAnimationInfo::operator=(const LinuxAnimationInfo &r)
{
   m_name = r.m_name;
   m_distanceCovered = r.m_distanceCovered;
   m_naturalDurationInMsec = r.m_naturalDurationInMsec;
...
HAnimClass* LinuxAnimationInfo::getAnimHandle() const
{ 
DEBUG_CRASH(("LinuxAnimationInfo::getAnimHandle not yet implemented!"));
return nullptr;
#if 0
#ifdef RETAIN_ANIM_HANDLES
   if (m_handle == NULL)
   {
      ...
      m_handle = LinuxDisplay::m_assetManager->Get_HAnim(m_name.str());
   ...
   return m_handle;
#else
   HAnimClass* handle = LinuxDisplay::m_assetManager->Get_HAnim(m_name.str());
...
//-------------------------------------------------------------------------------------------------
LinuxAnimationInfo::~LinuxAnimationInfo() 
{ 
#ifdef RETAIN_ANIM_HANDLES
   REF_PTR_RELEASE(m_handle); 
   m_handle = NULL; 
...
void ModelConditionInfo::preloadAssets( TimeOfDay timeOfDay, Real scale )
{
   ...
   {
      TheDisplay->preloadModelAssets( m_ini.m_modelName );
   }
...
(void) robj;
(void) scale;
DEBUG_CRASH(("ModelConditionInfo::validateCachedBones not yet implemented!"));
#if 0
   ...
   if (robj == NULL)
   {
      ...
      robj = LinuxDisplay::m_assetManager->Create_Render_Obj(m_modelName.str(), scale, 0);
...
//#endif
      ...
      if (fxBoneName.isNotEmpty() || recoilBoneName.isNotEmpty() || mfName.isNotEmpty() || plbName.isNotEmpty())
      {
         ...
         DEBUG_ASSERTCRASH(!(m_ini.m_modelName.isNotEmpty() && m_weaponBarrelInfoVec[wslot].empty()), ("*** ASSET ERROR: No fx bone named '%s' found in model %s!\n", fxBoneName.str(), m_ini.m_modelName.str()));
      }
   }
...
void ModelConditionInfo::validateTurretInfo() const
{
   ...
   for (int tslot = 0; tslot < MAX_TURRETS; ++tslot)
   {
      ...
      if (tur.m_turretAngleNameKey != NAMEKEY_INVALID)
      {
         ...
         {
            DEBUG_CRASH(("*** ASSET ERROR: TurretBone %s not found! (%s)\n", KEYNAME(tur.m_turretAngleNameKey).str(), m_ini.m_modelName.str()));
            tur.m_turretAngleBone = 0;
         }
      }
      else
      ...
      if (tur.m_turretPitchNameKey != NAMEKEY_INVALID)
      {
         ...
         {
            DEBUG_CRASH(("*** ASSET ERROR: TurretBone %s not found! (%s)\n", KEYNAME(tur.m_turretPitchNameKey).str(), m_ini.m_modelName.str()));
            tur.m_turretPitchBone = 0;
         }
      }
      else
...
{ 
#ifdef RETAIN_ANIM_HANDLES
   for (LinuxAnimationVector::const_iterator it2 = m_animations.begin(); it2 != m_animations.end(); ++it2)
   {
...
//-------------------------------------------------------------------------------------------------
const Vector3* LinuxModelDrawModuleData::getAttachToDrawableBoneOffset(const Drawable* draw) const
{
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDrawModuleData::buildFieldParse(void* what, MultiIniFieldParse& p) 
{
...
// FIXME: LinuxModelDrawModuleData::buildFieldParse
      // { "InitialRecoilSpeed",	INI::parseVelocityReal, NULL, offsetof(LinuxModelDrawModuleData, m_initialRecoil) },
...
{
   AnimParseType animType = (AnimParseType)(intptr_t)userData;
   ...
   LinuxAnimationInfo animInfo(animName, (animType == ANIM_IDLE), distanceCovered);
   ...
   {
      while (timesToRepeat--)
         self->m_ini.m_animations.push_back(animInfo);
   }
}
...
// FIXME: LinuxModelDrawModuleData::buildFieldParse
   ...
   // no, we do not currently require a default state, cuz it would break the existing INI
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::onDrawableBoundToObject()
{
DEBUG_CRASH(("LinuxModelDraw::onDrawableBoundToObject not yet implemented!"));
   getLinuxModelDrawModuleData()->validateStuffForTimeAndWeather(getDrawable(), 
                                 TheGlobalData->m_timeOfDay == TIME_OF_DAY_NIGHT, 
                                 TheGlobalData->m_weather == WEATHER_SNOWY);
}
...
void LinuxModelDraw::allocateShadows(void)
{
   ...
   if (m_shadow == NULL && m_renderObject && TheLinuxShadowManager && tmplate->getShadowType() != SHADOW_NONE)
   {	
...
#if defined(_DEBUG) || defined(_INTERNAL)	
void LinuxModelDraw::getRenderCostRecursive(RenderCost & rc,RenderObjClass * robj) const 
{
   if (robj == NULL) return;
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::adjustAnimSpeedToMovementSpeed()
{
...
{
   const LinuxModelDrawModuleData* d = getLinuxModelDrawModuleData();
   ...
   if (d->m_attachToDrawableBone.isNotEmpty())
   {
      ...
      {
         DEBUG_LOG(("m_attachToDrawableBone %s not found\n",getLinuxModelDrawModuleData()->m_attachToDrawableBone.str()));
      }
   }
#endif
...
{
   const TransitionMap& transitionMap = getLinuxModelDrawModuleData()->m_transitionMap;
...
//-------------------------------------------------------------------------------------------------
Real LinuxModelDraw::getCurrentAnimFraction() const
{
   if (m_curState != NULL
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::adjustAnimation(const ModelConditionInfo* prevState, Real prevAnimFraction)
{
   if (!m_curState)
      return;
   ...
   if (numAnims > 0)
   {
      ...
      const LinuxAnimationInfo& animInfo = m_curState->m_animations[m_whichAnimInCurState];
...
//-------------------------------------------------------------------------------------------------
Bool LinuxModelDraw::setCurAnimDurationInMsec(Real desiredDurationInMsec)
{
...
//-------------------------------------------------------------------------------------------------
Real LinuxModelDraw::getCurAnimDistanceCovered() const
{
...
/*
   ...
   DANGER WARNING READ ME
*/
void LinuxModelDraw::handleClientTurretPositioning()
{
...
//void LinuxModelDraw::handleClientFlagPositioning()
//{
//	const ModelConditionInfo::TurretInfo& tur = m_curState->m_turrets[tslot];
...
*/
void LinuxModelDraw::handleClientRecoil()
{
   const LinuxModelDrawModuleData* d = getLinuxModelDrawModuleData();
...
/*
   ...
   DANGER WARNING READ ME
*/
void LinuxModelDraw::recalcBonesForClientParticleSystems()
{
...
/*
   ...
   DANGER WARNING READ ME
*/
Bool LinuxModelDraw::updateBonesForClientParticleSystems()
{
   const Drawable* drawable = getDrawable();
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::setTerrainDecalSize(Real x, Real y)
{
   if (m_terrainDecal)
   {
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::setTerrainDecalOpacity(Real o)
{
   if (m_terrainDecal)
   {
...
void LinuxModelDraw::hideAllHeadlights(Bool hide)
{
   if (m_renderObject)
   {
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::hideAllMuzzleFlashes(const ModelConditionInfo* state, RenderObjClass* renderObject)
{
...
/** Change the model for this drawable. If color is non-zero, it will also apply team color to the
   model */
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::setModelState(const ModelConditionInfo* newState)
{
...
//		hideAllMuzzleFlashes(newState, m_renderObject);//moved to above
      ...
      if (m_renderObject &&
            ...
            !getLinuxModelDrawModuleData()->m_trackFile.isEmpty())
      {
         m_trackRenderObject = TheTerrainTracksRenderObjClassSystem->bindTrack(m_renderObject, 1.0f*MAP_XY_FACTOR, getLinuxModelDrawModuleData()->m_trackFile.str());
...
//-------------------------------------------------------------------------------------------------
Bool LinuxModelDraw::getProjectileLaunchOffset(
...
) const
{
   ...
   */
...
#if defined(_DEBUG) || defined(_INTERNAL)
   CRCDEBUG_LOG(("LinuxModelDraw::getProjectileLaunchOffset() for %s\n",
   ...
   const LinuxModelDrawModuleData* d = getLinuxModelDrawModuleData();
   //CRCDEBUG_LOG(("validateStuffs() from within LinuxModelDraw::getProjectileLaunchOffset()\n"));
   //DUMPREAL(getDrawable()->getScale());
   //BONEPOS_LOG(("validateStuffs() from within LinuxModelDraw::getProjectileLaunchOffset()\n"));
   //BONEPOS_DUMPREAL(getDrawable()->getScale());
...
//	if (isValidTimeToCalcLogicStuff())
//	{
//		CRCDEBUG_LOG(("LinuxModelDraw::getPristineBonePositionsForConditionState() - state = '%s'\n",
//			stateToUse->getDescription().str()));
//		//CRCDEBUG_LOG(("renderObject == NULL: %d\n", (stateToUse==m_curState)?(m_renderObject == NULL):1));
//	}

   //BONEPOS_LOG(("validateStuff() from within LinuxModelDraw::getPristineBonePositionsForConditionState()\n"));
   //BONEPOS_DUMPREAL(getDrawable()->getScale());
...
//	if (isValidTimeToCalcLogicStuff())
//	{
//		CRCDEBUG_LOG(("end of LinuxModelDraw::getPristineBonePositionsForConditionState()\n"));
//	}
...
// this method must ONLY be called from the client, NEVER From the logic, not even indirectly.
Bool LinuxModelDraw::clientOnly_getRenderObjBoundBox(OBBoxClass * boundbox) const
{
   if (!m_renderObject)
      return false;
...
//-------------------------------------------------------------------------------------------------
Int LinuxModelDraw::getCurrentBonePositions(
   const char* boneNamePrefix, 
   Int startIndex, 
   Coord3D* positions, 
   Matrix3D* transforms, 
...
//-------------------------------------------------------------------------------------------------
Bool LinuxModelDraw::handleWeaponFireFX(WeaponSlotType wslot, Int specificBarrelToUse, const FXList* fxl, Real weaponSpeed, const Coord3D* victimPos, Real damageRadius)
{
   DEBUG_ASSERTCRASH(specificBarrelToUse >= 0, ("specificBarrelToUse should now always be explicit"));
   ...
   if (info.m_recoilBone || info.m_muzzleFlashBone)
   {
      ...
      recoil.m_recoilRate = getLinuxModelDrawModuleData()->m_initialRecoil;
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::setAnimationLoopDuration(UnsignedInt numFrames)
{
...
*/
void LinuxModelDraw::setAnimationCompletionTime(UnsignedInt numFrames)
{
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::setAnimationFrame( int frame )
{
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::setPauseAnimation(Bool pauseAnim)
{
...
Real LinuxModelDraw::getAnimationScrubScalar( void ) const
{
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::rebuildWeaponRecoilInfo(const ModelConditionInfo* state) 
{
   Int wslot;
...
//-------------------------------------------------------------------------------------------------
void LinuxModelDraw::updateProjectileClipStatus( UnsignedInt shotsRemaining, UnsignedInt maxShots, WeaponSlotType slot )
{
   if ((getLinuxModelDrawModuleData()->m_projectileBoneFeedbackEnabledSlots & (1 << slot)) == 0)
...
/** CRC */
// ------------------------------------------------------------------------------------------------
void LinuxModelDraw::crc( Xfer *xfer )
{
...
/** Xfer method
   ...
   */
// ------------------------------------------------------------------------------------------------
void LinuxModelDraw::xfer( Xfer *xfer )
{
(void) xfer;
DEBUG_CRASH(("LinuxModelDraw::xfer not yet implemented!"));
#if 0
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
void LinuxModelDraw::loadPostProcess()
{
...
// ------------------------------------------------------------------------------------------------
void LinuxModelDrawModuleData::crc(Xfer* x)
{
   xfer(x);
}

// ------------------------------------------------------------------------------------------------
void LinuxModelDrawModuleData::xfer(Xfer* x)
{
...
// ------------------------------------------------------------------------------------------------
void LinuxModelDrawModuleData::loadPostProcess( void )
{
}
