...
//=============================================================================
/** Frees the OpenGL resources used to draw the terrain. */
//=============================================================================
void HeightMapRenderObjClass::freeIndexVertexBuffers(void)
{
	...
	if (m_vertexArrays) {
		glDeleteVertexArrays(m_numVertexBufferTiles, m_vertexArrays);
		delete m_vertexArrays;
		m_vertexArrays = nullptr;
	}
...
*/
//=============================================================================
Int HeightMapRenderObjClass::updateVB(GLuint vertexbuffer, char *data, Int x0, Int y0, Int x1, Int y1, Int originX, Int originY, WorldHeightMap *pMap, RefRenderObjListIterator *pLightsIterator)
{
	Int i,j;
...
DEBUG_LOG(("VBO %d: (%d, %d) to (%d, %d), origin = (%d, %d)\n", vertexbuffer, x0, y0, x1, y1, originX, originY));
		...
		for (j=y0; j<y1; j++)
		{
			...
			for (i=x0; i<x1; i++)
			{
				...
				for (Int lightIndex=0; lightIndex < TheGlobalData->m_data.m_numGlobalLights; lightIndex++)
				{
					lightPos=&TheGlobalData->m_data.m_terrainLightPos[lightIndex];
...
DEBUG_LOG(("Top left: (%.2f, %.2f, %.2f), (%.3f, %.3f), (%.3f, %.3f)\n", vb->x, vb->y, vb->z, vb->u1, vb->v1, vb->u2, vb->v2));
Real d {(vb->z + 24.0f) / 64.0f};
vb->r = d;
vb->g = d;
vb->b = d;
vb->a = 1.0f;
...
DEBUG_LOG(("Top right: (%.2f, %.2f, %.2f), (%.3f, %.3f), (%.3f, %.3f)\n", vb->x, vb->y, vb->z, vb->u1, vb->v1, vb->u2, vb->v2));
d = (vb->z + 24.0f) / 64.0f;
vb->r = d;
vb->g = d;
vb->b = d;
...
DEBUG_LOG(("Bottom right: (%.2f, %.2f, %.2f), (%.3f, %.3f), (%.3f, %.3f)\n", vb->x, vb->y, vb->z, vb->u1, vb->v1, vb->u2, vb->v2));
d = (vb->z + 24.0f) / 64.0f;
vb->r = d;
vb->g = d;
vb->b = d;
...
DEBUG_LOG(("Bottom left: (%.2f, %.2f, %.2f), (%.3f, %.3f), (%.3f, %.3f)\n", vb->x, vb->y, vb->z, vb->u1, vb->v1, vb->u2, vb->v2));
d = (vb->z + 24.0f) / 64.0f;
vb->r = d;
vb->g = d;
vb->b = d;
...
if (vb->z > maxZ) { maxZ = vb->z; }
		...
		size_t vertexBufferSize {VERTEX_BUFFER_TILE_LENGTH * 2 * VERTEX_BUFFER_TILE_LENGTH * 2 * sizeof(VERTEX_FORMAT)};
		glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
		glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(vertexBufferSize), data, GL_STATIC_DRAW);
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 11 * 4, reinterpret_cast<void*>(0));
		glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 11 * 4, reinterpret_cast<void*>(3 * 4));
		glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 11 * 4, reinterpret_cast<void*>(7 * 4));
		glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, 11 * 4, reinterpret_cast<void*>(9 * 4));
		glEnableVertexAttribArray(0);
		glEnableVertexAttribArray(1);
		glEnableVertexAttribArray(2);
		glEnableVertexAttribArray(3);
DEBUG_LOG(("VBO %d: %.3f, %.3f, %.3f - %.3f, %.3f, %.3f\n", vertexbuffer, minX, minY, minZ, maxX, maxY, maxZ));
		return 0; //success.
	}
	return -1;
}
...
(void) pLightsIterator;
	...
	{
		Int originY = j*VERTEX_BUFFER_TILE_LENGTH;	//location of this VB on the large full-size heightmap
		Int yMin, yMax;
		yMin = originY;
		if (y0>yMin) yMin = y0;
		...
		{
			Int originX = i*VERTEX_BUFFER_TILE_LENGTH;	//location of this VB on the large full-size heightmap
			Int xMin, xMax;
			xMin = originX;
			if (xMin<x0) xMin = x0;
			...
			Int index {j * m_numVBTilesX + i};
			glBindVertexArray(m_vertexArrays[index]);
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indexBuffer);
			GLuint vb = m_vertexBufferTiles[index];	//point to correct row/column of vertex buffers 
			char *pData = m_vertexBufferBackup[index];
			updateVB(vb, pData, xMin, yMin, xMax, yMax, originX, originY, pMap, pLightsIterator);
		}
	}
	glBindVertexArray(0);
#endif // if 0
...
DEBUG_LOG(("<<< Create big vertex buffer here >>>\n"));
		...
		for (size_t tileX {0}; tileX < tilesPerDimension; ++tileX) {
			...
			for (size_t y {0}; y < VERTEX_BUFFER_TILE_LENGTH; ++y) {
				for (size_t x {0}; x < VERTEX_BUFFER_TILE_LENGTH; ++x) {
					...
					if (pMap) {
						pMap->getUVData(heightOffsetX + x, heightOffsetY + y, U, V, HALF_RES_MESH);
						pMap->getAlphaUVData(heightOffsetX + x, heightOffsetY + y, UA, VA, alpha, &flipForBlend, HALF_RES_MESH);
					}
					...
					if (sz < min.z) { min.z = sz; }
					if (sx > max.x) { max.x = sx; }
					if (sy > max.y) { max.y = sy; }
					if (sz > max.z) { max.z = sz; }
					vertexBufferData[squareOffset + 0].x = sx;
					vertexBufferData[squareOffset + 0].y = sy;
					vertexBufferData[squareOffset + 0].z = sz;
					...
					vertexBufferData[squareOffset + 0].v2 = VA[0];
					// Top-right vertex.
					...
					if (sy > max.y) { max.y = sy; }
					if (sz > max.z) { max.z = sz; }
					vertexBufferData[squareOffset + 1].x = sx;
					vertexBufferData[squareOffset + 1].y = sy;
					vertexBufferData[squareOffset + 1].z = sz;
					...
					vertexBufferData[squareOffset + 1].v2 = VA[1];
					// Bottom-right vertex.
					hx = heightOffsetX + x + 1;
					hy = heightOffsetY + y;
					height = (hx < mapSize && hy < mapSize) ? pMap->getHeight(hx, hy) : 0;
					...
					if (sy > max.y) { max.y = sy; }
					if (sz > max.z) { max.z = sz; }
					vertexBufferData[squareOffset + 2].x = sx;
					vertexBufferData[squareOffset + 2].y = sy;
					vertexBufferData[squareOffset + 2].z = sz;
					...
					vertexBufferData[squareOffset + 2].v2 = VA[2];
					// Bottom-left vertex.
					hx = heightOffsetX + x;
					hy = heightOffsetY + y;
					height = (hx < mapSize && hy < mapSize) ? pMap->getHeight(hx, hy) : 0;
					...
					if (sy > max.y) { max.y = sy; }
					if (sz > max.z) { max.z = sz; }
					vertexBufferData[squareOffset + 3].x = sx;
					vertexBufferData[squareOffset + 3].y = sy;
					vertexBufferData[squareOffset + 3].z = sz;
	...
	glNamedBufferStorage(m_vertexBuffer, static_cast<GLsizeiptr>(vertexBufferSize), vertexBufferData, 0);
	...
	glVertexArrayAttribFormat(m_vertexArray, 1, 4, GL_FLOAT, GL_FALSE, 3 * 4);
	glVertexArrayAttribFormat(m_vertexArray, 2, 2, GL_FLOAT, GL_FALSE, 7 * 4);
	glVertexArrayAttribFormat(m_vertexArray, 3, 2, GL_FLOAT, GL_FALSE, 9 * 4);
	glVertexArrayAttribBinding(m_vertexArray, 0, 0);
	glVertexArrayAttribBinding(m_vertexArray, 1, 0);
	glVertexArrayAttribBinding(m_vertexArray, 2, 0);
	glVertexArrayAttribBinding(m_vertexArray, 3, 0);

	glVertexArrayVertexBuffer(m_vertexArray, 0, m_vertexBuffer, 0, 11 * 4);
	glVertexArrayElementBuffer(m_vertexArray, m_indexBuffer);

	TheDynamicMemoryAllocator->freeBytes(vertexBufferData);
...
//	Int	vertsPerColumn=y*2-2;
	...
	{
DEBUG_LOG(("<<< Create index buffer here >>>\n"));
...
#endif // if 0
		std::array<GLushort, VERTEX_BUFFER_TILE_LENGTH * VERTEX_BUFFER_TILE_LENGTH * 2 * 3> indexBufferData {};
		GLushort* ib {indexBufferData.data()};

		for (int j=0; j<(VERTEX_BUFFER_TILE_LENGTH*VERTEX_BUFFER_TILE_LENGTH*4); j+=VERTEX_BUFFER_TILE_LENGTH*4)
		{
			for (int i=j; i<(j+VERTEX_BUFFER_TILE_LENGTH*4); i+=4)	//4 vertices per 2x2 block
			{
				ib[0]=i;
				ib[1]=i+2;
				ib[2]=i+3;
		...
		glNamedBufferStorage(m_indexBuffer, std::size(indexBufferData) * sizeof(GLushort), indexBufferData.data(), 0);
		...
		//blocksize
		m_numVBTilesX = 1;
		for (int i = VERTEX_BUFFER_TILE_LENGTH + 1; i < x;)
		{
			i += VERTEX_BUFFER_TILE_LENGTH;
			++m_numVBTilesX;
		}
		m_numVBTilesY = 1;
		for (int j = VERTEX_BUFFER_TILE_LENGTH + 1; j < y;)
		{
		...
		m_x = x;
		m_y = y;
DEBUG_LOG(("<<< Create %d vertex buffers here >>>\n", m_numVertexBufferTiles));
		m_vertexArrays = NEW GLuint[m_numVertexBufferTiles];
		glGenVertexArrays(m_numVertexBufferTiles, m_vertexArrays);
		m_vertexBufferTiles = NEW GLuint[m_numVertexBufferTiles];
		glGenBuffers(m_numVertexBufferTiles, m_vertexBufferTiles);
		m_vertexBufferBackup = NEW char *[m_numVertexBufferTiles];

		size_t numVertex = VERTEX_BUFFER_TILE_LENGTH * 2 * VERTEX_BUFFER_TILE_LENGTH * 2;

DEBUG_LOG(("HeightMap: x = %d, y = %d, m_numVBTilesX = %d, m_numVBTilesY = %d, m_numBlockColumnsInLastVB = %d, m_numBlockRowsInLastVB = %d, m_numVertexBufferTiles = %d, numVertex = %d\n",
	x, y, m_numVBTilesX, m_numVBTilesY, m_numBlockColumnsInLastVB, m_numBlockRowsInLastVB, m_numVertexBufferTiles, numVertex));
...
//	float aspect = camera->Get_Aspect_Ratio();
	...
	if (m_map) {
		...
		if (HALF_RES_MESH) {
			newOrgX = (Int)((UnsignedInt)newOrgX & 0xFFFFFFFE);
			newOrgY = (Int)((UnsignedInt)newOrgY & 0xFFFFFFFE);
		}
...
void HeightMapRenderObjClass::renderTerrainPass(CameraClass* /* pCamera */)
{
	...
	m_stageZeroTexture->Peek_GL_Texture()->bind(0);
	m_stageZeroTexture->Peek_GL_Sampler()->bind(0);
	glBindVertexArray(m_vertexArray);
	for (int i {0}; i < m_tileCount; ++i) {
		glDrawElementsBaseVertex(GL_TRIANGLES, 32 * 32 * 2 * 3,  GL_UNSIGNED_SHORT, (GLvoid*)0, m_tiles[i].offset);
	}
	...
	for (Int j=0; j<m_numVBTilesY; j++)
		for (Int i=0; i<m_numVBTilesX; i++)
		{
			...
			Int index {j * m_numVBTilesX + i};
			glBindVertexArray(m_vertexArrays[index]);
			glDrawElements(GL_TRIANGLES, numPolys * 3, GL_UNSIGNED_SHORT, (GLvoid*)0);
#ifdef PRE_TRANSFORM_VERTEX
			if (m_xformedVertexBuffer && pass==0) {
				// Note - m_xformedVertexBuffer should only be used for non T&L hardware.  jba.
				DX8Wrapper::Apply_Render_State_Changes();
