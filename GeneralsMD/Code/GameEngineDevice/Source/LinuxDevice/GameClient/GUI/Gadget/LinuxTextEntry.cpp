...
//	Color constructColor = TheWindowManager->winMakeColor( 192, 0, 192, 255 );
   ...
   if ( TheIMEManager && TheIMEManager->isAttachedTo( window) && TheIMEManager->isComposing())
   {
      ...
      if ( e->secretText )
      {
         ...
         Int len = static_cast<Int>(composition.getLength()) + e->text->getTextLength();
         for ( int i = 0; i < len; i++ )
         {
            e->sText->appendChar( '*' );
         }
...
// LinuxGadgetTextEntryDraw ===================================================
/** Draw colored entry field using standard graphics */
//=============================================================================
void LinuxGadgetTextEntryDraw( GameWindow *window, WinInstanceData *instData )
{
   EntryData *e = (EntryData *)window->winGetUserData();
   ...
   // draw the edit text
   drawTextEntryText( window, instData, textColor, textBorder, compositeColor, compositeBorder, start.x, start.y, width, fontHeight );

}  // end LinuxGadgetTextEntryDraw

// LinuxGadgetTextEntryImageDraw ==============================================
/** Draw horizontal slider with user supplied images */
//=============================================================================
void LinuxGadgetTextEntryImageDraw( GameWindow *window, WinInstanceData *instData )
{
   EntryData *e = (EntryData *)window->winGetUserData();
   ICoord2D origin, size, start, end;
...
}  // end LinuxGadgetTextEntryImageDraw
