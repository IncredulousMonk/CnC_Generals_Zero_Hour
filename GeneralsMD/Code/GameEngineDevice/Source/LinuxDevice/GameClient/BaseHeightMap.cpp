...
// #include "LinuxDevice/GameClient/TerrainTex.h"
// #include "LinuxDevice/GameClient/LinuxDynamicLight.h"
// #include "LinuxDevice/GameClient/LinuxScene.h"
// #include "LinuxDevice/GameClient/LinuxTerrainTracks.h"
// #include "LinuxDevice/GameClient/LinuxBibBuffer.h"
// #include "LinuxDevice/GameClient/LinuxPropBuffer.h"
// #include "LinuxDevice/GameClient/LinuxTreeBuffer.h"
// #include "LinuxDevice/GameClient/LinuxRoadBuffer.h"
// #include "LinuxDevice/GameClient/LinuxBridgeBuffer.h"
// #include "LinuxDevice/GameClient/LinuxWaypointBuffer.h"
// #include "LinuxDevice/GameClient/LinuxCustomEdging.h"
// #include "LinuxDevice/GameClient/WorldHeightMap.h"
// #include "LinuxDevice/GameClient/LinuxShaderManager.h"
...
// #include "LinuxDevice/GameClient/LinuxPoly.h"
// #include "LinuxDevice/GameClient/LinuxCustomScene.h"
...
// #include "LinuxDevice/GameClient/LinuxSnow.h"
...
void BaseHeightMapRenderObjClass::doTheLight(VERTEX_FORMAT *vb, Vector3*light, Vector3*normal, RefRenderObjListIterator* /* pLightsIterator */, UnsignedByte alpha)
{
	...
	Real shade;
	shadeR = TheGlobalData->m_data.m_terrainAmbient[0].red;	//only the first terrain light contributes to ambient
	shadeG = TheGlobalData->m_data.m_terrainAmbient[0].green;
	shadeB = TheGlobalData->m_data.m_terrainAmbient[0].blue;
	...
	for (Int lightIndex=0; lightIndex < TheGlobalData->m_data.m_numGlobalLights; lightIndex++)
	{
		...
		if (shade > 1.0) shade = 1.0;
		if(shade < 0.0f) shade = 0.0f;
		terrainDiffuse=&TheGlobalData->m_data.m_terrainDiffuse[lightIndex];
	...
	if (m_useDepthFade && vb->z <= TheGlobalData->m_data.m_waterPositionZ)
	{	//height is below water level
		//reduce lighting values based on light fall off as it travels through water.
		float depthScale = (1.4f - vb->z)/TheGlobalData->m_data.m_waterPositionZ;
	...
	// shadeB*=255.0f;
	Real a {alpha / 255.0f};
	// vb->diffuse = REAL_TO_UNSIGNEDINT(shadeB) | (REAL_TO_UNSIGNEDINT(shadeG) << 8) | (REAL_TO_UNSIGNEDINT(shadeR) << 16) | ((UnsignedInt)alpha << 24);
	vb->r = shadeR;
	vb->g = shadeG;
	vb->b = shadeB;
	vb->a = a;
...
//  DEBUG_ASSERTCRASH( height < 30, ("SOMEBODY THINKS THE CLIENT HEIGHTMAP IS GOOD ENOUGH FOR LOGIC SAMPLING."));
	...
	if (normal) {
		...
		d8 = data[idx9+1];
		d9 = data[idx9];
		// FIXME: d10 is not used.  Is that correct?
		// d10 = data[idx3-1];
		d11 = data[idx0-1];
		...
		// Interpolate to get the smoothed value.
