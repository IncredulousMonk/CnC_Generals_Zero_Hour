...
void StickyBombUpdate::initStickyBomb( Object *target, const Object *bomber, const Coord3D *specificPos )
{
	...
	if( target )
	{
		...
		else if(target->isKindOf( KINDOF_IMMOBILE ) && bomber )
		{
			...
			pos.z += d->m_ini.m_offsetZ; // ride on the roof of the truck/tank
...
UpdateSleepTime StickyBombUpdate::update( void )
{
	...
	if( target )
	{
		...
		else // make the bomb follow the target around
		{
			...
			newPos.x = pos->x;
			newPos.y = pos->y;
			newPos.z = pos->z + d->m_ini.m_offsetZ;
...
void StickyBombUpdate::detonate()
{
	...
	if( data->m_ini.m_geometryBasedDamageWeaponTemplate )
	{
		...
		if( boobyTrappedObject )
		{
			...
			Real primaryDamage = data->m_ini.m_geometryBasedDamageWeaponTemplate->getPrimaryDamage(nullBonus);
			Real secondaryDamage = data->m_ini.m_geometryBasedDamageWeaponTemplate->getSecondaryDamage(nullBonus);
			Real primaryDamageRange = data->m_ini.m_geometryBasedDamageWeaponTemplate->getPrimaryDamageRadius(nullBonus);
			Real secondaryDamageRange = data->m_ini.m_geometryBasedDamageWeaponTemplate->getSecondaryDamageRadius(nullBonus);
			...
			damageInfo.in.m_damageType = data->m_ini.m_geometryBasedDamageWeaponTemplate->getDamageType();
			damageInfo.in.m_deathType = data->m_ini.m_geometryBasedDamageWeaponTemplate->getDeathType();
			damageInfo.in.m_sourceID = getObject()->getID();
			damageInfo.in.m_sourcePlayerMask = getObject()->getControllingPlayer()->getPlayerMask();
			damageInfo.in.m_damageStatusType = data->m_ini.m_geometryBasedDamageWeaponTemplate->getDamageStatusType();
			...
			if( data->m_ini.m_geometryBasedDamageFX )
			{
				// And we make FX based on that size too.
				FXList::doFXPos(data->m_ini.m_geometryBasedDamageFX, boobyTrappedObject->getPosition(), NULL, 0, NULL, secondaryDamageRange);
			}
		}
	}
