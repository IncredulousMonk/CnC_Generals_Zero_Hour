...
#include "LinuxDevice/GameClient/TileData.h"
...
public:
	...
	// No copies allowed!
	GDIFileStream(const GDIFileStream&) = delete;
	GDIFileStream& operator=(const GDIFileStream&) = delete;
...
WorldHeightMap::WorldHeightMap(ChunkInputStream *pStrm, Bool logicalDataOnly)
{
	...
	if (TheGlobalData && TheGlobalData->m_data.m_stretchTerrain) {
	...
	if (TheGlobalData && TheGlobalData->m_data.m_drawEntireTerrain) {
...
void WorldHeightMap::clearFlipStates(void) {
	if (m_cellFlipState) {
		memset(m_cellFlipState, 0, (size_t)(m_flipStateWidth*m_height));	//clear all flags
	}
}
...
Bool WorldHeightMap::getSeismicUpdateFlag(Int xIndex, Int yIndex) const
{
	...
	return m_seismicUpdateFlag[yIndex * (Int)m_seismicUpdateWidth + (xIndex >> 3)] & (1<<(xIndex&0x7));
}
void WorldHeightMap::setSeismicUpdateFlag(Int xIndex, Int yIndex, Bool value) 
{
	...
	UnsignedByte *curVal = &m_seismicUpdateFlag[yIndex * (Int)m_seismicUpdateWidth + (xIndex >> 3)];
	if (value) {
		*curVal |= (1<<(xIndex&0x7));
	}	else {
		*curVal &= ~(1<<(xIndex&0x7));
...
{
	if (m_seismicUpdateFlag) {
		memset(m_seismicUpdateFlag, 0, (size_t)((Int)m_seismicUpdateWidth * m_height));	//clear all flags
	}
}
...
Bool WorldHeightMap::ParseWorldDictDataChunk(DataChunkInput &file, DataChunkInfo* /* info */, void* /* userData */)
{
	...
	*MapObject::getWorldDict() = d;
	...
	if (exists) {
		TheWritableGlobalData->m_data.m_weather = (Weather) theWeather;
	}
	return true;
}
...
Bool WorldHeightMap::ParseBlendTileData(DataChunkInput &file, DataChunkInfo *info, void* /* userData */)
{
	...
	memset(m_cellFlipState, 0, (size_t)(numBytesX*numBytesY));	//clear all flags
	memset(m_cellCliffState, 0, (size_t)(numBytesX*numBytesY));	//clear all flags

	file.readArrayOfBytes((char*)m_tileNdxes, m_dataSize * (Int)sizeof(Short));
	file.readArrayOfBytes((char*)m_blendTileNdxes, m_dataSize * (Int)sizeof(Short));
	if (info->version >= K_BLEND_TILE_VERSION_6) {
		file.readArrayOfBytes((char*)m_extraBlendTileNdxes, m_dataSize * (Int)sizeof(Short));
		...
		if (!TheGlobalData->m_data.m_use3WayTerrainBlends)
			memset(m_extraBlendTileNdxes, 0, (size_t)m_dataSize * sizeof(Short));
	} 
	if (info->version >= K_BLEND_TILE_VERSION_5) {
		file.readArrayOfBytes((char*)m_cliffInfoNdxes, m_dataSize * (Int)sizeof(Short));
	} 
...
// --> file loading here
	...
	for (i=1; i<m_numBlendedTiles; i++) {
		...
		m_blendedTiles[i].horiz = (UnsignedByte)file.readByte();
		m_blendedTiles[i].vert = (UnsignedByte)file.readByte();
		m_blendedTiles[i].rightDiagonal = (UnsignedByte)file.readByte();
		m_blendedTiles[i].leftDiagonal = (UnsignedByte)file.readByte();
		m_blendedTiles[i].inverted = (UnsignedByte)file.readByte();
		//Allow clearing of extra blend tiles via ini and resaving of map.
		//Useful for flushing out initial maps made with buggy 3-way blending.
		if (!TheGlobalData->m_data.m_use3WayTerrainBlends)
			m_blendedTiles[i].inverted &= (UnsignedByte)~FLIPPED_MASK;	//filter out extra flips from 3-way

		if (info->version >= K_BLEND_TILE_VERSION_3) {
			m_blendedTiles[i].longDiagonal = (UnsignedByte)file.readByte();
		} else {
...
Bool WorldHeightMap::getUVForTileIndex(Int ndx, Short tileNdx, float U[4], float V[4], Bool fullTile)
{
	...
	if ((ndx<m_dataSize) && m_tileNdxes) {
		...
		U[0] = nU; U[1] = xU; U[2] = xU; U[3] = nU;
		V[0] = xV; V[1] = xV; V[2] = nV; V[3] = nV;
		if (TheGlobalData && !TheGlobalData->m_data.m_adjustCliffTextures) {
			return false;
		}
		if (nU==0.0) {
...
{
	if (TheWritableGlobalData->m_data.m_textureReductionFactor) {
		if (TheWritableGlobalData->m_data.m_textureReductionFactor>1) {
			pixelsPerCell /= 4;
		} else {
			pixelsPerCell /= 2;
		}
...
void WorldHeightMap::getTerrainColorAt(Real x, Real y, RGBColor *pColor)
{
	...
	TileData *pTile = getSourceTile((UnsignedInt)tileNdx);
	if (pTile) {
		...
		if (blendTileNdx>0 && blendTileNdx < NUM_BLEND_TILES) {
			...
			if (getRawTileData(pBlend->blendNdx, width, s_blendBuffer, DATA_LEN_BYTES)) {
				...
				for (i=0; i<limit; i++) {
					...
					a = *pAlpha; pAlpha += 4;
					*pDestData = ((b*a)/255) + (((*pDestData)*(255-a))/255); pDestData++;
					*pDestData = ((g*a)/255) + (((*pDestData)*(255-a))/255); pDestData++;
					*pDestData = ((r*a)/255) + (((*pDestData)*(255-a))/255); pDestData++;
																				 ...
																				 UnsignedByte *buffer, Int bufLen)
{
	...
	if (pSrc && pSrc->hasRGBDataForWidth(2*width)) {
		...
		for (j=0; j<width; j++) {
			...
			memcpy(pDestData, pSrc, (size_t)width*TILE_BYTES_PER_PIXEL);
		}
		return(true);
	}
	return(false);
