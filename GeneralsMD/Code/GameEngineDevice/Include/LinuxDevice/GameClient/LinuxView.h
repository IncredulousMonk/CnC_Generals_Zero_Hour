...
struct TMoveAlongWaypointPathInfo {
   ...
   ParabolicEase ease {0.0f, 0.0f};          // Ease in/out function.
};
...
{
private:
   std::forward_list<RenderObjClass*> m_3dObjects {};
...
public:
   ...
   // No copies allowed!
   LinuxView(const LinuxView&) = delete;
   LinuxView& operator=(const LinuxView&) = delete;
   ...
   // SubsystemInterface hooks (to enable performance monitoring):
...
#endif // if 0
   virtual void setAngleAndPitchToDefault();    ///< Set the view angle back to default 
