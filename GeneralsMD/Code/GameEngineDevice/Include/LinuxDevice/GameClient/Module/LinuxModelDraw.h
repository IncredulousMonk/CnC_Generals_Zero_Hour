...
#ifndef __LinuxModelDraw_H_
#define __LinuxModelDraw_H_
...
public:
   LinuxAnimationInfo(const AsciiString& name, Bool isIdle, Real distanceCovered);
   LinuxAnimationInfo(const LinuxAnimationInfo &r);
   LinuxAnimationInfo &operator=(const LinuxAnimationInfo& r);
...
typedef std::vector<LinuxAnimationInfo> LinuxAnimationVector;
...
struct ModelConditionInfo
{
   ...
   {
      Int            m_recoilBone {};           ///< the Linux bone for this barrel (zero == no bone)
   ...
   // MG: Cannot apply offsetof to ModelConditionInfo, so had to move data into an embedded struct.
   struct IniData
   {
   ...
   inline const ModelConditionFlags& getNthConditionsYes(Int i) const { return m_conditionsYesVec.data()[i]; }
#if defined(_DEBUG) || defined(_INTERNAL)
...
public:
   ...
   // ugh, hack
   virtual const LinuxModelDrawModuleData* getAsLinuxModelDrawModuleData() const { return this; }
...
class LinuxModelDraw : public DrawModule, public ObjectDrawInterface
{
   ...
   MAKE_STANDARD_MODULE_MACRO_WITH_MODULE_DATA(LinuxModelDraw, LinuxModelDrawModuleData)
...
// srj sez: not sure if this is a good idea, for net sync reasons...
   ...
   ///@todo: I had to make this public because LinuxDevice needs access for casting shadows -MW 
